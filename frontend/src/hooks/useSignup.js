    ...
    const signup = async ({leagueID, fname, lname, username, password, email, userType }) => {
        ...
        await axios.post(`/api/user/join-league`, requestBody).then(response => {
            ...
            if (response.status === 200){
                ...
                dispatch({type: 'LOGIN', leagueID: response.data.leagueID, userID: response.data.userID, leagueName: response.data.leagueName, username: username, userToken: response.data.userToken});
